ModifyEvent(54, 33, 0, -2, 0, 0, 1230, 0, 0, 0, 0, -2, -2);
ModifyEvent(-2, 0, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
ModifyEvent(-2, 1, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
Talk("怎么這么多人？", 0, 1);
SubMapViewFromTo(26, 49, 22, 31);
Talk("哪一個先來決一死戰！", 50, 0);
Talk("喬兄弟，契丹和大宋勢不兩立，咱們公而忘私，老哥哥要得罪了！", 93, 1);
Talk("絕交酒也喝過了，干什么還稱兄道弟？看招！", 50, 0);
PlayAnimation(2, 8472, 8498);
DarkScene();
LightScene();
Talk("大伙兒靠著墻壁，莫要亂斗", 82, 1);
Talk("我來領教領教聚賢莊游氏雙雄的手段．", 50, 0);
PlayAnimation(2, 8472, 8498);
DarkScene();
LightScene();
Talk("好極，送了這兩件利器給我", 50, 0);
Talk("兄弟，師父說道：”盾在人在，盾亡人亡”．", 82, 1);
Talk("哥哥，今日遭此奇恥大辱，咱從前兒倆更有什么臉面活在世上？", 82, 5);
DarkScene();
LightScene();
Talk("＜哎呀！游氏雙雄竟然為了兩件兵器而自刎，真是太傻了！＞", 0, 1);
Talk("喬峰逼死游氏雙雄，大家一起上，為游氏雙雄報仇！", 4, 1);
Talk("幫助喬峰請按”Ｙ”幫助眾人請按”Ｎ”", 0, 1);
if AskRest() == true then goto  label0 end;
    AddMorality(-5);
    Talk("喬峰！你這契丹狗！虧我上次與你喝酒還跟你稱兄道弟沒想到你竟然逼死游氏雙雄看我今天替天行道！", 0, 1);
    DarkScene();
    OldSetScenePosition(22, 30);
    LightScene();
    Talk("是你？好！喬某跟你喝一杯絕情酒，然后決一死戰！", 50, 0);
    Talk("在下雖不濟，但也不肖跟你這契丹狗喝酒．廢話少說，受死吧！", 0, 1);
    Talk("（無奈）呵呵，想不到我喬峰今日一戰，竟然一個朋友都沒有了．既然如此，動手吧！", 50, 0);
    if TryBattle(84) == true then goto label1 end;
        Dead();
        exit();
::label1::
        LightScene();
        Talk("鼎鼎大名的喬幫主想不到今日會死在我一個無名小卒手中吧？", 0, 1);
        Talk("！！！", 117, 0);
        DarkScene();
        ModifyEvent(-2, 22, 1, -2, 0, 0, 0, 5296, 5296, 5296, 0, -2, -2);
        LightScene();
        if TryBattle(85) == true then goto label2 end;
            Dead();
            exit();
::label2::
            ModifyEvent(-2, 22, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
            LightScene();
            Talk("＜那個黑衣人是誰？＞喬峰，這回看你往哪跑！", 0, 1);
            Talk("要殺就動手吧！", 50, 0);
            Talk("是否殺了喬峰？", 0, 3);
            if AskRest() == true then goto  label3 end;
                Talk("．．．你走吧．．．", 0, 1);
                Talk("．．．．．．", 50, 0);
                DarkScene();
                ModifyEvent(-2, 2, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                ModifyEvent(-2, 23, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                ModifyEvent(52, 11, 1, -2, 862, 0, 0, 6120, 6120, 6120, 0, -2, -2);
                SetTowards(3);
                LightScene();
                Talk("你怎么放走喬峰這狗賊？", 79, 0);
                if TryBattle(86) == true then goto label4 end;
                    Dead();
                    exit();
::label4::
                    ModifyEvent(-2, 3, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 4, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 5, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 6, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 7, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 8, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 9, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 10, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 11, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 12, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 13, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 14, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 15, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 16, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 17, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 18, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 19, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 20, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 21, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    LightScene();
                    exit();
::label3::
                    AddMorality(-3);
                    DarkScene();
                    ModifyEvent(-2, 2, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    SetTowards(3);
                    LightScene();
                    Talk("多謝少俠襄助，今日才能除去喬峰這狗賊！", 79, 0);
                    Talk("喬峰乃蠻族狗賊，人人得而誅之！", 0, 1);
                    Talk("少俠后會有期！", 79, 0);
                    DarkScene();
                    ModifyEvent(-2, 3, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 4, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 5, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 6, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 7, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 8, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 9, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 10, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 11, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 12, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 13, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 14, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 15, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 16, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 17, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 18, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 19, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 20, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    ModifyEvent(-2, 21, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                    LightScene();
                    exit();
::label0::
                    AddMorality(3);
                    OldSetScenePosition(22, 28);
                    SetTowards(3);
                    Talk("喬幫主，我來幫你退敵！", 0, 1);
                    if TryBattle(86) == true then goto label5 end;
                        Dead();
                        exit();
::label5::
                        LightScene();
                        Talk("我們這些人怎么會輸呢！再上！", 79, 0);
                        if TryBattle(86) == true then goto label6 end;
                            Dead();
                            exit();
::label6::
                            ModifyEvent(-2, 3, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 4, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 5, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 6, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 7, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 8, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 9, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 10, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 11, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 12, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 13, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 14, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 15, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 16, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 17, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 18, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 19, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 20, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 21, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(-2, 23, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            LightScene();
                            SetTowards(0);
                            Talk("多謝小兄弟相助！", 50, 0);
                            Talk("說哪里話，在下一直欽佩喬幫主為人，其實在下有一個心愿，想與喬幫主結為異性兄弟．不知喬幫主．．．", 0, 0);
                            Talk("其實我也正有此意．賢弟！", 50, 0);
                            Talk("大哥！不知道大哥有什么打算．", 0, 1);
                            Talk("我想先到雁門關看看我父親留下的遺言，然后去尋找帶頭大哥．", 50, 0);
                            Talk("我陪大哥前往如何？", 0, 1);
                            Talk("我想一個人靜靜．況且賢弟還要去尋找東西，我還是不要浪費賢弟的時間了．", 50, 0);
                            Talk("大哥怎么知道我在找東西？", 0, 1);
                            Talk("怎么說我以前也是武林第一大幫的幫主啊，每天武林發生的事都會有人來向我報告", 50, 0);
                            Talk("好！大哥，那我們后會有期！", 0, 1);
                            Talk("后會有期！", 50, 0);
                            DarkScene();
                            ModifyEvent(-2, 2, 0, -2, 0, 0, 0, 0, 0, 0, 0, -2, -2);
                            ModifyEvent(6, 7, 1, -2, 775, 0, 0, 6292, 6292, 6292, 0, -2, -2);
                            ModifyEvent(6, 8, 1, -2, 775, 0, 0, 6118, 6118, 6118, 0, -2, -2);
                            LightScene();
exit();
