if isUsingItem(183) == false then
exit();
end;
AddMorality(-14);
AddItemWithoutHint(183, -1);
Talk("喬峰，你看這是什麼？", 0, 1, 1964);
Talk("是什麼？", 50, 0, 1965);
Talk("你看了便知道．", 0, 1, 1966);
DarkScene();
PlayMusic(9);
LightScene();
Talk("這．．這是真的嗎？", 50, 0, 1967);
Talk("少林方丈親筆寫的，會是假的嗎！", 0, 1, 1968);
Talk("我．．我．．我不是漢人．．．．我是契丹人．．．．", 50, 0, 1969);
Talk("喬峰，你是契丹人，非我漢族人士，怎可擔任這丐幫幫主一職，保管”天龍八部”一書呢！", 0, 1, 1970);
Talk("你要我怎麼做！", 50, 0, 1971);
Talk("辭去丐幫幫主之位，交出”天龍八部”一書．", 0, 1, 1972);
Talk("好！我今天就辭去這丐幫幫主之位，”天龍八部”一書你拿去吧．", 50, 0, 1973);
DarkScene();
ModifyEvent(-2, -2, 0, 0, -1, -1, -1, -1, -1, -1, -2, -2, -2);
LightScene();
AddItem(147, 1);
Talk("得來真是全不費功夫．", 0, 1, 1974);
Talk("閣下來此，揭發喬峰的真實身份，讓我丐幫不致誤奉一契丹人為幫主，很是感激．", 93, 0, 1975);
Talk("沒什麼，這是中原武林的大事，小弟應該做的．", 0, 1, 1976);
Talk("但是，拜你所賜，我丐幫也將因此事貽笑武林．你拿的”天龍八部”是丐幫鎮幫之寶，還請閣下留下．", 93, 0, 1977);
Talk("怎麼可以，我好不容易才拿到手的．", 0, 1, 1978);
Talk("那只好得罪了．", 93, 0, 1979);
if TryBattle(84) == false then
Dead();
exit();
end;
LightScene();
ModifyEvent(-2, 2, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 3, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 4, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 6, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 7, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 8, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 9, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 10, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 11, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 12, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 13, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 15, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 16, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 17, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 18, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
ModifyEvent(-2, 19, -2, -2, 532, -1, -1, -2, -2, -2, -2, -2, -2);
AddFame(5);