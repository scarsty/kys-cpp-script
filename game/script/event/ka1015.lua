...
Talk("門總算開了，終於可以回去了．可是，心裡面好像怪怪的，總覺得好像少了件什麼事．．．．．．", 0, 1, 2915);
if CheckRoleMorality(0, 0, 50) == false then
Talk("小子，別想走．", 60, 0, 2916);
Talk("真的發生事情了．", 0, 1, 2917);
DarkScene();
...
SetTowards(1);
LightScene();
Talk("我就知道，每次在結局前，總是少不了一場大混戰．", 0, 1, 2918);
Talk("小子，你武功那麼強，一定是從”十四天書”中領悟到什麼奧妙．", 67, 0, 2919);
Talk("快將它說出來，否則．．．", 71, 0, 2920);
Talk("否則怎樣？你們個個都是小俠我的手下敗將，還想怎樣？", 0, 1, 2921);
Talk("一個人打不過你，那十個人呢？", 22, 0, 2922);
Talk("不會吧！你們個個都是武林中有名的夙宿，不會聯合起來打我一個人吧？傳了出去，你們以後怎麼做人？", 0, 1, 2923);
Talk("事到如此，還管得了這麼許多嗎？", 19, 0, 2924);
Talk("何況，把你殺了後，也沒人會傳出去了．", 51, 0, 2925);
Talk("別說廢話了．動手吧！", 62, 0, 2926);
if TryBattle(133) == false then
Dead();
exit();
end;
LightScene();
Talk("哼！算你厲害．我們走．", 60, 0, 2927);
DarkScene();
...
LightScene();
PlayWave(23);
Talk("總算料理完畢了．咦！好像又有聲音．", 0, 1, 2928);
DarkScene();
SetSubMapLayerData(-2, 1, 18, 25, 4062);
SetSubMapLayerData(-2, 1, 18, 26, 4062);
LightScene();
Talk("怎麼門又關起來了．這次大概真的結束了．走吧，我得趕快了．", 0, 1, 2929);
exit();
end;
Talk("小子，你想就這樣子走了嗎？", 57, 0, 2930);
Talk("真的發生事情了．", 0, 1, 2917);
DarkScene();
...
LightScene();
Talk("我就知道，每次在結局前，總是少不了一場大混戰．", 0, 1, 2918);
Talk("在這武林中，你幹下了多少壞事．", 69, 0, 2931);
Talk("看看你的道德指數吧，你都幹了些什麼？", 3, 0, 2932);
Talk("練武是為了什麼？行俠仗義，濟弱扶傾的目的你都忘了嗎？", 55, 0, 2933);
Talk("你在武林中混了這麼久，盡學一些壞東西．", 64, 0, 2934);
Talk("是我們錯了，沒能把你教好．", 56, 0, 2935);
Talk("但我們也不能讓你就這樣一走了之．", 70, 0, 2936);
Talk("別怪我們出手太重，你留在這世間也只是個禍害．", 50, 0, 2937);
Talk("動手吧．", 69, 0, 2938);
Talk("阿彌陀佛！", 70, 0, 2939);
...
LightScene();
Talk("我們已經盡力了，還是無法消滅你這禍害．", 55, 0, 2940);
Talk("阿彌陀佛！這是眾生的浩劫呀！", 70, 0, 2941);
Talk("殺不了你這個大魔頭．唉！或許是天意吧！我們走吧．", 69, 0, 2942);
DarkScene();
ModifyEvent(-2, 25, 0, 0, -1, -1, -1, -1, -1, -1, -2, -2, -2);
ModifyEvent(-2, 26, 0, 0, -1, -1, -1, -1, -1, -1, -2, -2, -2);
ModifyEvent(-2, 27, 0, 0, -1, -1, -1, -1, -1, -1, -2, -2, -2);
