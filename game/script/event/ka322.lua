Talk("少俠，老夫聽說你獨自攻上這黑木崖來，特別率我五嶽派門徒趕上來幫你．東方不敗那魔頭呢？", 19, 0, 1124);
Talk("唉！你們怎麼跟電影劇情一樣，事情都結束後才出現．東方不敗那個變態傢伙，已經被我”幹掉”了．", 0, 1, 1125);
Talk("真是如此！少俠又替我武林正派立了一項大功．", 19, 0, 1126);
Talk("那裡，那裡．", 0, 1, 1127);
Talk("那不知少俠是否有看到一本書？", 19, 0, 1128);
Talk("什麼書？", 0, 1, 1129);
Talk("”笑傲江湖”！", 19, 0, 1130);
Talk("當然有啊！我辛苦了半天，還不都是為了它．任老頭將它送給我了．", 0, 1, 1131);
Talk("這又是另一項功勞了．少俠真當為我武林同道所敬仰．此書乃是我五嶽劍派許多前輩當年拼命所得到的，後來不幸被魔教奪去．我五嶽派這幾年發奮圖強，與魔教周旋到底，為的就是將此書奪回．想不到今日少俠大顯神威，從魔教手中將此書奪回，我五嶽派全體上下無限感激．現在就請少俠將此書還與我五嶽派．", 19, 0, 1132);
Talk("＜說了半天，原來也是想要  ”笑傲江湖”這本書＞不過據我所知，這本書的原主人也不是五嶽劍派的人．你們前輩只是暫時得到過．今天，我的任務就是要將這些書送回原處，你們以後也不用再為此煩心．更何況這書也不是你們想像中的什麼高深武功秘笈，它只不過是本故事書罷了．", 0, 1, 1133);
Talk("莫非，少俠是想將此書佔為己有．還是你已與魔教勾結了？", 19, 0, 1134);
Talk("岳先生，你別給我亂安罪名了．這書真的不是你們想像中的什麼高深武功秘笈，它只不過是本故事書罷了．", 0, 1, 1135);
Talk("看來少俠入魔已深，非得要逼我們動手不可．", 19, 0, 1136);
Talk("我看是你岳不群入魔已深．那日嵩山大會上我就有點被”耍”的感覺．今天，你為了搶這本”笑傲江湖”又想不擇手段．今日你怎會突然出現，分明就是早已算計好的，想等我跟東方不敗鶴蚌相爭，你好來個漁翁得利．我看你不是”君子劍”，分明是個”偽君子”！", 0, 1, 1137);
Talk("你．．．你說什麼！", 19, 0, 1138);
if InTeam(35) then
Talk("師父，別再執迷下去了．這書的確不是武功秘笈，只是一本普通的書．", 35, 1, 1139);
end;
Talk("岳先生，回頭是岸．", 0, 1, 1140);
Talk("別說這麼多的廢話，五嶽派的人聽著，給我殺，今日不留一個活口．", 19, 0, 1141);
...
LightScene();
Talk("五嶽派的各位，今日多有冒犯之處，請多包涵．", 0, 1, 1142);
Talk("哼！我們走．", 19, 0, 1143);
DarkScene();
...
LightScene();
Talk("想不到在找尋這本書的過程中，讓我看到人性中為了爭權奪利所展現的醜陋面．不管是正派中的左冷禪與岳不群，還是邪派中的任我行與東方不敗，都是一樣的．", 0, 1, 1144);
