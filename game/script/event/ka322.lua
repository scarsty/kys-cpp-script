Talk("少俠，老夫聽說你獨自攻上這黑木崖來，特別率我五嶽派門徒趕上來幫你．東方不敗那魔頭呢？", 19, 0);
Talk("唉！你們怎麼跟電影劇情一樣，事情都結束後才出現．東方不敗那個變態傢伙，已經被我”幹掉”了．", 0, 1);
...
Talk("這又是另一項功勞了．少俠真當為我武林同道所敬仰．此書乃是我五嶽劍派許多前輩當年拼命所得到的，後來不幸被魔教奪去．我五嶽派這幾年發奮圖強，與魔教周旋到底，為的就是將此書奪回．想不到今日少俠大顯神威，從魔教手中將此書奪回，我五嶽派全體上下無限感激．現在就請少俠將此書還與我五嶽派．", 19, 0);
Talk("＜說了半天，原來也是想要  ”笑傲江湖”這本書＞不過據我所知，這本書的原主人也不是五嶽劍派的人．你們前輩只是暫時得到過．今天，我的任務就是要將這些書送回原處，你們以後也不用再為此煩心．更何況這書也不是你們想像中的什麼高深武功秘笈，它只不過是本故事書罷了．", 0, 1);
Talk("莫非，少俠是想將此書佔為己有．還是你已與魔教勾結了？", 19, 0);
...
Talk("岳先生，回頭是岸．", 0, 1);
Talk("別說這麼多的廢話，五嶽派的人聽著，給我殺，今日不留一個活口．", 19, 0);
...
LightScene();
Talk("想不到在找尋這本書的過程中，讓我看到人性中為了爭權奪利所展現的醜陋面．不管是正派中的左冷禪與岳不群，還是邪派中的任我行與東方不敗，都是一樣的．", 0, 1);
